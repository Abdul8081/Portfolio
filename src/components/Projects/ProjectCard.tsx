...
import TiltCardProject from '../TiltCardProject'
...
const ProjectCard: React.FC<ProjectCardProps> = ({ data }) => {
  ...
  return (
    <TiltCardProject className="bg-secondary border-border flex flex-col justify-between rounded-[14px] border p-5">
      <div className="flex items-start justify-between gap-2">
        <div className="flex-1">
          <div className="flex flex-col flex-wrap gap-3 sm:flex-row sm:items-center">
            ...
            {type && (
              <span
                className={`hidden sm:inline h-7 w-fit rounded-md bg-[#FFFFFF1A] p-1 text-sm ${
          ...
          <ul className="mt-3 flex flex-col flex-wrap gap-2 sm:flex-row sm:gap-4">
            ...
            {siteAge && (
              <li className="hidden sm:flex gap-2">
                <Image src={Timer} alt={siteAge} className="size-[18px] md:size-5" />
                <span className="text-neutral text-sm">{siteAge}</span>
              </li>
            )}
